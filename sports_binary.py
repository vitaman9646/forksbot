"""
Sports Binary Scanner v1.5
───────────────────────────
Изменения vs v1.3:
  • volume check на каждый исход (min_volume из CFG)
  • stale check через updatedAt
...
def extract_tag_strings(tags_raw) -> List[str]:
    if not tags_raw:
        return []
    result = []
    for tag in tags_raw:
...
class SportsArb:
    ...
    def format_alert(self):
        m = self.match
        min_vol = min(m.volume_a, m.volume_b)
        return (
    ...
    def to_dict(self):
        return {
            ...
            "team_a": self.match.team_a,
            "team_b": self.match.team_b,
            "arb_type": self.arb_type,
            "yes_a": self.match.yes_a,
            "yes_b": self.match.yes_b,
            "sum": self.sum_prices,
            "net_edge_pct": self.net_edge_pct,
            "volume_a": self.match.volume_a,
            "volume_b": self.match.volume_b,
            "timestamp": self.timestamp,
        }


def is_sports(event: dict) -> bool:
...
def parse_binary(event: dict) -> Optional[BinaryMatch]:
    ...
    for m in open_markets:
        ...
        if yes_price <= 0.01:
            return None
        # v1.5: проверяем минимальный объём
        if vol < CFG.MIN_VOLUME:
            logger.debug(f"Sports skip (low vol ${vol:.0f}): {question[:40]}")
            return None
...
def detect_arb(match: BinaryMatch) -> Optional[SportsArb]:
...
async def scan_sports(session) -> List[SportsArb]:
    try:
        params = {
            "active": "true",
            "closed": "false",
    ...
    candidates = [
        e for e in events
        if is_sports(e) and len([
            m for m in e.get("markets", [])
            if not m.get("closed", False)
        ]) == 2
    ]
    logger.info(
        f"Sports binary: {len(candidates)} candidates "
    ...
    for event in candidates:
        ...
        if arb:
            ...
            logger.info(
                ...
                f"Sum: ${arb.sum_prices:.4f} | "
                f"Edge: {arb.net_edge_pct:.2f}%"
            )
    return results
